...
(define all-reachable
  (lambda (base-ptrs)
    (reachable-help base-ptrs '())))

(define reachable-help
  (lambda (queue visited)
    (if (null? queue)
        visited
        (let ([cur-ptr (car queue)])
          (if (ptr-member? cur-ptr visited)
              (reachable-help (cdr queue) visited)
              (reachable-help (ptr-union (ptr-pointers cur-ptr) (cdr queue)) (cons cur-ptr visited)))))))
...
(define ptr-member?
  (lambda (ptr ls)
    (if (null? ls)
        ...
        (if (ptr=? ptr (car ls))
            #t
            (ptr-member? ptr (cdr ls))))))

(define ptr-union
  (lambda (l1 l2)
    (if (null? l1)
        l2
        (if (or (null-ptr? (car l1)) (ptr-member? (car l1) l2))
            (ptr-union (cdr l1) l2)
            (cons (car l1) (ptr-union (cdr l1) l2))))))
...
(define garbage-collect
  (lambda (basis-set)
    (let* ([sorted (sort-by-loc (all-reachable (filter (lambda (x) (not (null-ptr? x))) basis-set)))]
           [h (new-ptrs sorted)])
      (set! next-index 0)
      (compact sorted h)
      (map (lambda (x) (if (= x -1) (ptr -1) (ptr (hash-ref h x)))) (map ptr-index basis-set)))))
    ...
    (let ([hash (make-hash)])
          (let loop ([ls r]
            ...
            (if (null? ls)
                hash
                (begin
                  (hash-set! hash (ptr-index (car ls)) cur)
                  (loop (cdr ls) (+ cur (ptr-length (car ls))))))))))
...
(define compact
  (lambda (r hash)
    (if (null? r)
        (void)
        (begin
          (let ([p (ptr-index (car r))])
            (write-array!
             (append (list (vector-ref memory p))
                     (map (lambda (x) (if (= x -1) -1 (hash-ref hash x))) (map ptr-index (ptr-pointers (ptr p))))
                     (ptr-values (ptr p)))
             next-index))
          (compact (cdr r) hash)))))
...
(define-memstruct my-type ptrs: (p1) vals: (v1 v2 v3))
...
;;   (lambda ()
;;     (reset-memory 20)
;;     (let* ([p1 (put! 'test-type null-ptr null-ptr 100)]
;;            [p2 (put! 'my-type p1 5 32 123)]
;;            [p3 (put! 'my-type null-ptr 6 66 666)]
;;            [p4 (put! 'test-type p2 p3 77)])
;;       (set-field! 'my-type 'p1 p2 p3)
;;       (void))))
;; 
;; (our-test)
;; 
;; memory
;; 
;; (garbage-collect (list (ptr 4)))
;; 
;; memory

;;(let-values ([(p1 p2 p3 p4 p5 p6) (scenario3)]) (garbage-collect (list p1)))
